...
class RFBDataLoader:
    ...
    def setup_directories(self) -> Tuple[Path, Path]:
        ...
        caminho_zip = os.getenv('OUTPUT_FILES_PATH', 'Dados_CNPJ_Arquivos')
        caminho_extraido = os.getenv('EXTRACTED_FILES_PATH', 'Dados_CNPJ_Extraidos')

        output_path = Path(caminho_zip)
        extract_path = Path(caminho_extraido)
